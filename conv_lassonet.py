...
from lassonet.module import hier_prox
...
class ConvLassoNet(nn.Module):
    def __init__(self, 
        ...
        self.fc1 = nn.Linear((self.h_out2 // 2)* (self.w_out2 // 2) * self.out_channels2, 200)
        self.fc2 = nn.Linear(200, self.D_out)
        self.features = nn.Sequential(self.conv1, self.relu, self.maxpool, self.conv2, self.relu, self.maxpool)
    ...
    def prox(self, lr):
        ...
        for j in range(getattr(self, self.sparse_layer).out_channels):
            ...
            threshold = torch.clamp(torch.linalg.norm(w_j) - self.lambda_ * lr, min=0.)
